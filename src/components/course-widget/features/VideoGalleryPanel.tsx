...
export default function VideoGalleryPanel() {
  ...
  return (
    <div className="p-4 h-full flex flex-col space-y-4">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-[#e8e1d8] pb-3 shrink-0">
        {activeVideoLesson ? (
          <button 
      ...
      {activeVideoLesson && activeVideoUrl ? (
        <div className="flex-1 flex flex-col gap-4 min-h-0 animate-fade-in">
          ...
          <div className="shrink-0 pb-4">
            <span className="text-xs font-bold text-[#bf2f1f] tracking-wide uppercase">Module {activeVideoLesson.moduleNo}</span>
            <h2 className="text-xl md:text-2xl font-bold text-[#000000] mt-1">{activeVideoLesson.topicName}</h2>
          </div>
        </div>
      ) : (
        <div className="flex-1 overflow-y-auto min-h-0 pr-2 pb-4 animate-fade-in">
