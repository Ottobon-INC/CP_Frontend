      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr_1fr] gap-8">
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white rounded-3xl shadow-sm p-8 border border-gray-100 hover:shadow-md transition-shadow">
            <h2 className="text-lg font-bold text-retro-teal text-retro-teal mb-8">Submit a New Review</h2>
            ...
            <div className="mb-8">
              ...
              <div className="relative group">
                <select 
                  ...
                  className="w-full h-14 border border-gray-200 rounded-2xl px-6 py-1 text-[0.95rem] font-semibold bg-gray-50 appearance-none cursor-pointer focus:border-retro-salmon outline-none transition-all shadow-sm"
                >
                  {FEEDBACK_COURSES.map(c => <option key={c}>{c}</option>)}
                </select>
                <i className="fas fa-chevron-down absolute right-6 top-1/2 -translate-y-1/2 text-gray-300 pointer-events-none group-hover:text-retro-salmon transition-colors"></i>
              </div>
            </div>
              ...
              <div className="flex gap-2 text-2xl">
                {[1, 2, 3, 4, 5].map((star) => (
                  <i 
                    ...
                    className={`fas fa-star cursor-pointer transition-all ${star <= rating ? 'text-retro-salmon scale-110' : 'text-gray-200 hover:text-orange-200'}`}
                  ></i>
                ))}
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                {[
                  ...
                  <div key={cat.id} className="flex items-center justify-between group">
                    <span className="text-[0.85rem] font-bold text-gray-600 group-hover:text-retro-teal transition-colors">{cat.label}</span>
                    <div 
            ...
            <div className="mb-8">
              <textarea
                ...
                className="w-full border border-gray-200 rounded-2xl p-6 text-[0.95rem] text-retro-teal resize-none min-h-[160px] focus:border-retro-salmon outline-none shadow-inner bg-gray-50 focus:bg-white transition-all font-medium"
              ></textarea>
            </div>

            <button className="w-full bg-retro-salmon hover:bg-orange-700 text-white font-extrabold rounded-2xl py-5 text-[1.05rem] transition-all transform hover:-translate-y-1 shadow-lg flex items-center justify-center gap-3">
        ...
        <div className="flex flex-col gap-8">
          <div className="bg-white rounded-[2rem] shadow-sm p-8 border border-gray-100 hover:shadow-md transition-shadow">
            <h2 className="text-lg font-bold text-retro-teal mb-8">Feedback Insights</h2>
            <div className="bg-gray-50 rounded-2xl p-6 mb-6 border border-gray-100">
               ...
               <div className="flex items-center justify-center gap-4">
                 <span className="text-4xl font-extrabold text-retro-teal">4.2</span>
                 <div className="flex text-retro-salmon text-xl gap-0.5">
            ...
            <div className="px-2">
              ...
              <span className="text-3xl font-extrabold text-retro-teal">8</span>
            </div>
          </div>

          <div className="bg-white rounded-3xl shadow-sm p-8 border border-gray-100 hover:shadow-md transition-shadow">
            <h2 className="text-lg font-bold text-retro-teal mb-2">Quick Reaction</h2>
          ...
          <div className="bg-white rounded-3xl shadow-sm p-8 border border-gray-100 hover:shadow-md transition-shadow">
            <h2 className="text-lg font-bold text-retro-teal mb-2">Suggestion Box</h2>
            <p className="text-[0.8rem] text-gray-400 font-bold mb-6">What can we improve?</p>
            <textarea
              value={suggestionText}
              onChange={(e) => setSuggestionText(e.target.value)}
              className="w-full border border-gray-200 rounded-2xl p-5 text-[0.9rem] text-retro-teal resize-none min-h-[140px] focus:border-retro-salmon outline-none shadow-sm font-medium bg-gray-50 focus:bg-white transition-all shadow-inner"
            ></textarea>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] shadow-sm p-8 border border-gray-100 hover:shadow-md transition-shadow flex flex-col">
          <h2 className="text-lg font-bold text-retro-teal text-retro-teal mb-8">Feedback History</h2>
            ...
            {FEEDBACK_HISTORY.map((item, idx) => (
              <div key={idx} className="group pb-8 border-b border-gray-50 last:border-0 last:pb-0">
                <div className="flex justify-between items-start mb-2">
                  <h4 className="font-extrabold text-[1.05rem] text-retro-teal leading-tight group-hover:text-retro-salmon transition-colors">{item.course}</h4>
                ...
                <div className="flex gap-1 text-[0.7rem] text-retro-salmon mb-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <button className="text-[0.75rem] font-extrabold text-gray-300 hover:text-retro-salmon transition-colors flex items-center gap-1.5 px-2 py-1 rounded-lg hover:bg-orange-50">
